...
"""
    ...
    def _analyze_heading_and_rewrite(
        ...
        try:
            ...
            valid_headings = self.dedupe_preserve_order(raw_headings)
    ...
    def _build_prompt(self, question: str, sources: List, context: str) -> str:
        ...
        for i, src in enumerate(sources):
            try:
                ...
                self.logger.error(f"Error processing source {i} for prompt: {str(e)}")
                self.logger.error(f"Source {i} details: {src}")
                import traceback
    ...
    def _get_recent_context(self, history: List[Dict], last_n: int = 3) -> str:
        ...
        for i in range(0, len(recent), 2):
            try:
                ...
                self.logger.warning(f"Error processing context message {i}: {str(e)}")
                continue
